...
<body id="fondo">
	...
	<?php
		...
		$resultado = mysqli_query($conexion,"SELECT * FROM ventas WHERE id_venta = $id_venta");
		$numreg = mysqli_num_rows($resultado);
		$numreg=mysqli_fetch_assoc($resultado);		
	?>
	<header id="header">
  <font color="white"><h2 class="animated fadeIn">Ventas</h2></font>
</header>
<br>
<br>
<br>
<br>
<br>
	<form action="ModifVP.php?id_venta=<?php echo $numreg['id_venta']?>" method="POST" class="form">									
		<center><img src="../Diseño/imagenes/book.png" style=" width: 80px;height: 80px;border: 0;"></center>
		<center><h1 id="t3">Ventas</h1></center>
		<center>
			<br><br>
			<label class="et">Fecha</label> <br>
			<input class="intext" type="text" name="fecha" value="<?php echo $numreg['fecha']; ?>" required title="Completa este campo" /> <br> <br>
